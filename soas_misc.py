...
from soas_data_processing import rhyme_marker
from soas_rnetwork_test import get_poem_name_given_number
from soas_rnetwork_test import is_line_a_rhyming_line
from soas_rnetwork_test import rnetwork
from soas_rnetwork_test import rn_rhyme_pairs
from soas_rnetwork_test import get_rhyme_type_if_it_is_there
#from soas_data_processing import get_phonological_data_dir
...
def get_soas_code_dir():
    return filename_storage.get_soas_code_dir()
...
def create_single_file_for_downloaded_ocr_data():
    ...
    base_dir = os.path.join(filename_storage.get_soas_code_dir(), 'data', '先秦漢魏晉南北朝詩')
...
def get_soas_data_dir():
    return filename_storage.get_soas_data_dir()


def readin_baxter1992_rhyme_data():
    funct_name = 'readin_baxter1992_rhyme_data()'
    data_file = os.path.join(filename_storage.get_soas_data_dir(), 'Baxter1992.tsv')
...
def readin_chars_missing_schuessler_data():
    ...
    input_file = os.path.join(filename_storage.get_phonological_data_dir(), 'chars_missing_schuessler_data.txt')
...
def get_ordered_list_of_gsc_numbers():
    ...
    input_file = os.path.join(filename_storage.get_phonological_data_dir(), 'max_gsc_per_bu.txt')
    if not os.path.isfile(input_file):
        print(funct_name + ' ERROR: Invalid input file: ' + input_file)
        return []
    data = readlines_of_utf8_file(input_file)
...
def get_list_of_chars_missing_sch_data_ordered_by_gsc_num():
    ...
    gsc2char_list_dict = {}
    output_file = os.path.join(filename_storage.get_phonological_data_dir(), 'missing_schuessler_char_list.txt')
    if os.path.isfile(output_file):
        os.remove(output_file)
...
def transform_missing_sch_file2_ordered_list():
    ...
    input_file = os.path.join(filename_storage.get_phonological_data_dir(), 'missing_schuessler_char_list.txt')
    output_file = os.path.join(filename_storage.get_phonological_data_dir(), 'missing_schuessler_char_list2.txt')
...
def transform_missing_sch_file3_ordered_list():
    ...
    input_file = os.path.join(filename_storage.get_phonological_data_dir(), 'char2gsc.txt')
    output_file = os.path.join(filename_storage.get_phonological_data_dir(), 'missing_schuessler_char_list3.txt')
    if not if_file_exists(input_file, funct_name):
        return
    delete_file_if_it_exists(output_file)
    raw_data = readlines_of_utf8_file(input_file)
...
def readin_local_variants():
    ...
    if local_var2normal_char_dict:
        return
    input_file = os.path.join(filename_storage.get_soas_code_dir(), 'hanproj', 'hanproject', 'local_variants.txt')
    if not if_file_exists(input_file, funct_name):
        return
    data = readlines_of_utf8_file(input_file)
    for d in data:
...
def semiautomatically_approximate_schuessler_late_han():
    ...
    input_file = os.path.join(filename_storage.get_phonological_data_dir(), 'need_shengfu_file.txt')
    output_file = os.path.join(filename_storage.get_phonological_data_dir(), 'char2gsc.txt')
